...
def _pick_column(df: pd.DataFrame, candidates) -> str:
    ...
    for c in candidates:
        ...
        if k in colmap:
            return colmap[k]
    raise ValueError(f"Missing required column. Expected one of {candidates}. Present: {list(df.columns)}")
...
def load_cost_centers_csv() -> pd.DataFrame:
    ...
    df = pd.read_csv(CSV_CC_LOOKUP, dtype=str, keep_default_na=False, encoding="utf-8-sig").fillna("")
...
def load_vol_desc_excel() -> pd.DataFrame:
    ...
    try:
        ...
        try:
            ...
            vv_col = _pick_column(df, ["Volume Description","Vol Description","VolDesc","VOLUME_DESC"])
            ...
            pass  # return raw if headers don't match expectations
        return df
    except Exception as e:
    ...
    def _worker():
        while True:
            try:
                ...
                log.exception("Failed computing next refresh time; defaulting to 24h: %s", e)
                wait_s = 24 * 3600
...
# ---------------------------------------------------------------------
# Preload orchestration compatible with Flask 3.x + Waitress
# ---------------------------------------------------------------------
...
def _init_preload_once():
    ...
    with _preload_lock:
        ...
        try:
            ...
            log.exception("Startup preload failed: %s", e)

def _kickoff_preload_in_background_on_import():
    # Kick off immediately at module import so it works under waitress
    t = threading.Thread(target=_init_preload_once, daemon=True, name="InitialPreloadKickoff")
    t.start()

_kickoff_preload_in_background_on_import()
...
def index():
    # Ensure preload happened (guard for unusual import orders)
    _init_preload_once()
...
# ---------------------------------------------------------------------
# Entrypoint (for flask run / python app.py). Waitress imports module only.
# ---------------------------------------------------------------------
